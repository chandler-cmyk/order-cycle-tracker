...
// ── Diagnostic: unit breakdown by source ───────────────────────────────────────
// GET /api/debug/units?start=&end=&category=Preroll
// Shows gross invoice units, CN deductions, SR deductions, and net per item name.
app.get('/api/debug/units', (req, res) => {
  try {
    ...
    const cnRows = db.prepare(`
      SELECT cni.name, SUM(cni.quantity) AS qty
      FROM credit_notes cn JOIN credit_note_items cni ON cn.creditnote_id = cni.creditnote_id
      WHERE cn.date BETWEEN ? AND ? AND cn.status NOT IN ('void','draft') AND cni.category = ?
      GROUP BY cni.name ORDER BY qty DESC
    `).all(s, e, catFilter);

    const srRows = db.prepare(`
      SELECT sri.name, SUM(sri.quantity) AS qty
      FROM sales_returns sr JOIN sales_return_items sri ON sr.salesreturn_id = sri.salesreturn_id
      WHERE sr.date BETWEEN ? AND ? AND sr.status NOT IN ('void','draft') AND sri.category = ?
      GROUP BY sri.name ORDER BY qty DESC
    `).all(s, e, catFilter);

    const invTotal = invRows.reduce((s, r) => s + r.qty, 0);
    const cnTotal  = cnRows.reduce((s, r) => s + r.qty, 0);
    const srTotal  = srRows.reduce((s, r) => s + r.qty, 0);

    res.json({
      category: catFilter, period: `${s} → ${e}`,
      summary: { invoiceUnits: invTotal, cnDeduction: cnTotal, srDeduction: srTotal, net: invTotal - cnTotal - srTotal },
      invoiceItems: invRows,
      cnItems: cnRows,
      srItems: srRows,
    });
  } catch (err) {
